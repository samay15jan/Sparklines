...
const Header = ({ data, image, type, name, artistName, year, time, songCount, followerCount, dominantColor }) => {
    ...
    <div>
      {data &&
        <div className='flex'>
          {image &&
            <img
              className={type ? 'drop-shadow-md w-48 h-48 rounded-lg': 'drop-shadow-md w-48 h-48 rounded-full'}
              src={image}
          ...
          <div className='relative z-1 ml-6 mt-16'>
            ...
            <div className='flex my-5'>
              ...
              {time && <div className='ml-1 text-sm font-medium'> &#128900; {time}</div>}
              {songCount && <div className='ml-1 text-sm font-medium'> &#128900; {songCount} Songs</div>}
              {followerCount && <div className='ml-1 text-md font-medium'>{followerCount} monthly listeners</div>}
            </div>
          </div>
        </div>
      }
